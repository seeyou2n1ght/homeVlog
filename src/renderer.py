...
def _stage_source_for_render(source: str | Path) -> Path | None:
    ...
    stage_dir = TEMP_DIR / "_render_stage"
    try:
    ...
    try:
        ...
        get_perf().add(PerfRecord(
            ...
            extra={"size_mb": round(source_stat.st_size / 1024**2, 1)},
        ))
        return staged
    except OSError as exc:
...
def build_batch_render(batch_segs, bi, enc_for_batch, fps, width, height, seg_cfg, out_cfg, audio_cfg, date, cam_index, rows):
    ...
    if virtual_mixed:
        prebuilt_plan = build_compact_virtual_concat_plan(
            ...
            dynamic_coalesce_gap_s=render_cfg.get("dynamic_coalesce_gap_s", 120.0),
            dynamic_window_max_s=render_cfg.get("dynamic_window_max_s", 600.0),
        )
        decode_ratio = prebuilt_plan.decoded_duration / max(0.001, prebuilt_plan.source_duration)
        max_ratio = float(render_cfg.get("virtual_concat_max_decode_ratio", 0.65))
...
def _run_batch_render(input_files, filter_complex, output_path, encoder, fps, out_cfg, audio_cfg, date, cam_index, batch_idx=0, pure_static_files=None, expected_duration=None, input_args=None, fingerprint_salt="", prepare_input=None) -> str | None:
    ...
    if encoder == "qsv":
        ...
        io_sem = VideoLease(get_nv_semaphore(), 1 if input_args is not None else len(input_files))
...
def _build_enc_args(encoder, out_cfg):
    if encoder == "qsv":
        ...
        nv = out_cfg.get("nv", {})
        return ["-c:v", nv.get("codec", "hevc_nvenc"), "-preset", nv.get("preset", "p1"), "-cq", str(nv.get("cq", 28)), "-maxrate", nv.get("maxrate", "4M"), "-bufsize", nv.get("bufsize", "8M"), "-g", "120", "-forced-idr", "1", "-pix_fmt", "yuv420p"]
