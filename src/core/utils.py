...
def load_config(config_path: str | Path | None = None, reload: bool = False) -> dict:
    ...
    if getattr(_self, "CONFIG_PATH", None) != _config.CONFIG_PATH:
        _config.CONFIG_PATH = _self.CONFIG_PATH
    if getattr(_self, "SETTINGS", None) is not _config.SETTINGS:
        _config.SETTINGS = _self.SETTINGS
    try:
        res = _config.load_config(config_path=config_path, reload=reload)
...
def cleanup_stale_logs(retention_days: int = 7) -> int:
    """清理 logs/ 各子目录下超过 retention_days 天的历史日志文件，防止磁盘无限膨胀。"""
    if not LOGS_DIR.exists():
        return 0
    now = time.time()
    cutoff = now - (retention_days * 86400)
    removed = 0
    for path_item in LOGS_DIR.rglob("*"):
        if path_item.is_file() and path_item.suffix in {".log", ".jsonl", ".json"}:
            try:
                if path_item.stat().st_mtime < cutoff:
                    path_item.unlink()
                    removed += 1
            except OSError:
                pass
...
def setup_logging(level_override: int | None = None) -> logging.Logger:
    """
    初始化全新的多目标分流日志体系：
    1. 主运行日志：logs/runs/homevlog_{timestamp}.log (默认 INFO)
    2. 独立错误排错日志：logs/error/error_{timestamp}.log (仅 WARNING/ERROR/CRITICAL)
    3. 结构化 JSONL 事件流：logs/events/events_{timestamp}.jsonl
    ...
    for d in [runs_dir, error_dir, events_dir, perf_dir]:
        d.mkdir(parents=True, exist_ok=True)

    cleanup_stale_logs(retention_days=log_cfg.get("retention_days", 7))

    timestamp = time.strftime("%Y%m%d_%H%M%S")
    log_file = runs_dir / f"homevlog_{timestamp}.log"
    err_file = error_dir / f"error_{timestamp}.log"
    jsonl_file = events_dir / f"events_{timestamp}.jsonl"
...
def cleanup_temp_artifacts(clean_batches: bool = False) -> int:
    ...
    if not TEMP_DIR.exists():
        return 0
    patterns = [
        "*.tmp.mp4", "_batch*.tmp.mp4", "_fc_*.txt", "_stderr_*.log",
        ".concat_*.txt", "_edl_*.ffconcat", "_render_stage/*.mp4",
        "_render_stage/*.part", "staging/*.mp4", "staging/*.part",
        "scripts/*", "_batch*.json.tmp",
    ]
    if clean_batches:
        patterns.extend([
            "_batch*.mp4", "_batch*.mp4.json",
            "batches/*.mp4", "batches/*.mp4.json",
        ])
...
def cleanup_staging_files() -> int:
    """清理本地预暂存目录下的已下载文件，彻底释放本地 SSD 临时存储。"""
    removed = 0
    stage_dirs = [TEMP_DIR / "_render_stage", TEMP_DIR / "staging"]
            ...
            for f in s_dir.iterdir():
                if f.is_file():
                    try:
                        ...
                        pass
    if removed:
        logging.getLogger("homevlog").info("staging cleanup: removed %d cached NAS files", removed)
    return removed
