...
启动 → Scanner扫描全部文件(date范围可能跨度多周)
     ...
     → 任意时刻Ctrl+C或断电：
        下次启动时，完美读取当天的所有任务：
        - 完全没碰过(PENDING) → 预筛队列
        - 预筛完(SUSPICIOUS)未分析 → 分析队列
        - 已确认(STATIC/ANALYZED) → 直接进入最终的渲染等待队列
     → 重启进程无缝衔接，不丢失任何已分析的中间进度
```

**场景B: 当天处理完成后，又有新素材加入**

```
启动 → Scanner扫描 → 发现新文件，INSERT
     → Pipeline 检查某天(date-cam)已 COMPLETED
     → 二次校验：查询该天是否有 PENDING 的新文件
     → 若有新文件：自动重置 COMPLETED 状态为 PENDING
     → 结合新旧文件的状态流无缝拼接，重新合并输出最新的 Vlog
```

### 11.3 独立的总结报告与性能日志
- **总结报告**: 每次运行（渲染完成或跳过渲染）后，在 `reports/` 目录下生成时间戳命名的独立 txt 报告，详情包含静止时长、动态时长及各段的时间戳。
- **性能日志**: 运行结束后会在 `logs/` 下生成精确到秒的 `perf_*.json` 文件，绝不覆盖，方便性能追溯分析。

### 11.4 与旧方案SQLite设计的差异
