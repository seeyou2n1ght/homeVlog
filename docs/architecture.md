...
### 2.5 自适应分块帧差法 (微动捕获)
针对纯 YOLO 深度学习在“极缓微动（如婴儿翻身）”、“红外噪点引发的 BBox 抖动”等场景下的盲区，V2.4 引入了可选的自适应分块帧差法：
- **异构解耦部署**: 帧差计算前置于 `DecoderWorker` 线程中，利用 CPU 解码等待间隙执行纯 NumPy 矩阵运算。完全避免阻塞 GPU Worker，维持 GPU 的满载吞吐。
- **动态背景建模**: 采用平滑学习率 (`alpha`) 更新背景，并对当前帧执行 16x16 宏块的方差建模，彻底免疫夜间 CMOS 红外热噪，成功累积并捕获低帧率下的极慢动作。
- **多模态语义融合**: 像素微动与 YOLO 语义强绑定。只有在 YOLO 识别到关注目标的前提下，像素的变动才会经由状态机的“漏桶防抖算法”最终确认为有效运动，杜绝了光变误报。
